...
class ConversationStore:
    ...
    def attach_file(
        ...
        # Store the actual filesystem path so the AI agent can read the file
        stored_file = file_path or filename

        # Avoid duplicate entries
        if stored_file not in uploaded_files:
    ...
    def remove_attachment(
        ...
        """
        Remove an attachment from the current conversation.

        This removes the file/image from the conversation context only.
        It does NOT delete the actual file from storage.
        """

        conv = ConversationStore.get_conversation(
            company,
            username,
            conversation_id
        )
        ...
        # Get only the filename, even if frontend/backend sends a full path
        ...
        existing_files = conv.get("uploaded_files", [])

        conv["uploaded_files"] = [
            file_entry
            for file_entry in existing_files
            if Path(str(file_entry)).name != requested_filename
        ]
        ...
        existing_images = conv.get("uploaded_images", [])

        conv["uploaded_images"] = [
            image_entry
            for image_entry in existing_images
            if Path(str(image_entry)).name != requested_filename
        ]
        ...
        return ConversationStore.update_conversation(
            company,
            username,
            conversation_id,
            conv
