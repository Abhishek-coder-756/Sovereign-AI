...
def classify_intent(
    ...
    """
    if file_path and not uploaded_files:
        uploaded_files = [Path(file_path).name]
    q = (query or "").strip()
    q_lower = q.lower()
...
def create_plan(state: AgentState) -> AgentState:
    ...
    # Synchronize file_path and uploaded_files
    file_path = state.get("file_path", "")
    if file_path and not uploaded_files:
        uploaded_files = [Path(file_path).name]
        state["uploaded_files"] = uploaded_files
    elif uploaded_files and not file_path:
        cand_p = find_document_path(uploaded_files[-1], state.get("company", "MRPL"))
        if cand_p:
            state["file_path"] = str(cand_p)

    intent = classify_intent(query, image_path, history, uploaded_files, file_path=state.get("file_path"))
    state["intent"] = intent
...
def spreadsheet_node(state: AgentState) -> AgentState:
    ...
    parts = query.split()
    file_path = ""
    if state.get("file_path") and state["file_path"].lower().endswith((".csv", ".xlsx", ".xls")):
